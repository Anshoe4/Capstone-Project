...
df <- df %>%
  ...
  arrange(FY_YEAR) %>%
  mutate(
    Comp_Growth = (sa_compensation_to_employees/ lag(sa_compensation_to_employees,3))^(1/3) - 1
  ) %>%
  ungroup()
df <- df %>%
  mutate(
    Comp_Growth = ifelse(is.infinite(Comp_Growth) | is.nan(Comp_Growth), NA, Comp_Growth)
  )
df <- df %>%
  mutate(industry_category = as.numeric(as.factor(co_industry_gp_code_y)))  # Create industry categories
df_filtered <- df %>%
  filter((PE_transaction == 1 & FY_YEAR == TX_YEAR) | (PE_transaction == 0))
treatment_firms <- df_filtered %>%
  filter(PE_transaction == 1) %>%
  distinct(sa_company_name, industry_category, Comp_Growth, TX_YEAR)
df_filtered <- df_filtered %>%
  filter(!is.na(Comp_Growth))

# Nearest Neighbor Matching 
match_model <- matchit(PE_transaction ~ Comp_Growth + industry_category + FY_YEAR + sa_total_assets,
                       data = df_filtered,
                       method = "nearest",
                       ratio = 1,
...
```{r compmodel,results='asis'}
library(dplyr)
library(ggplot2)
library(fixest)  # For running fixed effects DiD regression
...
model= feols(sa_compensation_to_employees ~ Post+PE_transaction+ PE_transaction * Post | industry_category + subclass, data = df_matched2)

etable(
  model,
  dict = c(
    sa_compensation_to_employees= "Compensation Paid to Employees (in $ million)",
    Post = "Post-Buyout Period",
    PE_transaction = "PE-Acquired Firm",
    "Post:PE_transaction" = "Treatment Effect (Post × PE)"
  ),
  tex = TRUE,
  title = "Difference-in-Differences (FE) Regression Results: Effect of Private Equity on Compensation Paid",
  digits = 2
)
``` 
...
#Aggregate trend plot
industry_average <- df_matched2 %>%
  group_by(Time_Category, PE_transaction) %>%
  summarise(avg_comp = mean(sa_compensation_to_employees, na.rm = TRUE), .groups = "drop")
p <- ggplot(industry_average, aes(
  ...
  labs(
    title = "Average Compensation Trends: PE vs Non-PE Firms",
    subtitle = "Time-Adjusted Comparison Relative to Transaction Year",
    x = "Years Relative to Buyout",
    y = "Average Total Compensation Paid (in $M)"
  ) +
...
p_industry <- ggplot(industry_avg, aes(
  ...
  labs(
    title = "Average Total Compensation Trends by Industry",
...
# Delta bar plots
outcomes <- c("sa_compensation_to_employees")
...
ggplot(summary_df, aes(x = variable, y = diff, fill = Group)) +
  ...
  labs(
    title = "Change in Compensation (T+2 - T-2)",
    x = "",
    y = "Difference",
    fill = ""
  ) +
...
ggplot(plot_data, aes(x = Time_Category, y = sa_compensation_to_employees, color = Group)) +
  ...
  labs(
    title = "Parallel Compensation Trends for 4 Random Treatment Control Pairs ",
